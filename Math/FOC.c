...
static void Angle_Generator_Update(void) {
    ...
    // RPM -> rad/s: omega = rpm * 2PI / 60
    omega_rad_s = FOC.Ramp_Speed * (_2PI / 60.0f);

    // 简单的速度积分: Theta = Theta + Omega * dt
    FOC.Ramp_Angle += omega_rad_s * FOC_DT;
...
void FOC_Init(void) {
    ...
    FOC.Ramp_Angle = 0.0f;
    FOC.Ramp_Speed = 0.0f;
    FOC.Ramp_Speed_Min = 30.0f;    // 从很低电角速度起步 (RPM)
    FOC.Ramp_Accel = 400.0f;       // 电角加速度 (RPM/s)
